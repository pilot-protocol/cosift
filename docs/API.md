...
| `mmr` | float in [0,1] | — (off) | Maximal Marginal Relevance diversification. λ=1 → pure relevance (no-op); λ=0 → pure diversity. 0.5–0.7 is a typical "relevance with some diversification" range. Requires HNSW graph loaded + embedder configured (the embedder requirement is waived on `/find_similar?url=X` URL-mode — anchor vector comes from the graph). Applied after rerank, before truncation to k. Adds `+mmr:λ` to the retriever label when it fires. Supported by /search, /answer, /research, /find_similar. |
| `decay` | float > 0 (days) | — (off) | Time-decay half-life. Multiplies each hit's score by `exp(-ln(2) · age_days / decay)` so a doc from `decay` days ago is worth 0.5x, 2×`decay` is 0.25x, etc. Hits without a known `PublishedAt` are unchanged (no signal → no penalty). Applied before rerank so the rerank pool reflects both quality and recency. Adds `+decay:Hd` to the retriever label. Bounded at 36500 (100 years). Supported on /search. |
