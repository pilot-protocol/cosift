...
| `expand` | string | false | `true` / `hyde` → HyDE passage appended to q. `paraphrase` (/search + /answer) → N chat-generated paraphrases, BM25 each, RRF-fuse. No-op when no chat client is configured. |
