...
| `mmr` | float in [0,1] | — (off) | Maximal Marginal Relevance diversification. λ=1 → pure relevance (no-op); λ=0 → pure diversity. 0.5–0.7 is a typical "relevance with some diversification" range. Requires HNSW graph loaded + embedder configured. Applied after rerank, before truncation to k. Adds `+mmr:λ` to the retriever label when it fires. Supported on /search. |
