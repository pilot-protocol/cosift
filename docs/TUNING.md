...
- **Unknown `?expand=` / `?sort=` values silently use the default.** Iter added warnings (`expand=foo is not a known strategy`, `sort=newest is not a known mode`). Pass valid values from `{hyde, paraphrase}` and `{relevance, date_desc, date_asc}`.
- **`?include_domains=https://example.com/foo` matches nothing.** The dot-boundary matcher expects bare hostnames; passing a URL silently filters everything out. catches this with a warning when any entry contains `/` or `://`.
