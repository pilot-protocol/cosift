...
func (s *pebbleHTTP) fetchCommunityReceipt(ctx context.Context, id, raw string, artifact *crawler.LocalArtifact) (crawler.ContributionReceipt, error) {
	if id == "" {
	...
	record := communityReceiptRecord{PayloadHash: hex.EncodeToString(hash[:])}
